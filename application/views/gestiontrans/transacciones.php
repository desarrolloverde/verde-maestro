...
<div class="container">
	<div class="col-md-8 center-block no-float">
		<ul class="nav nav-tabs">
		    ...
		    <li <?php echo ($sttrans==trim($datostab->id)) ?  'class="active tab-success"' :  '' ?>>
		    	<?php $tabid= (preg_match('/,/', $datostab->id)) ? urlencode($datostab->id) : $datostab->id;
		    	//$tabid=trim($tabid); ?>
		    	<a href="<?php echo base_url('index.php/gestiontransaccion/listadoTransacciones/'.$tabid); ?>"><?php echo $datostab->status.": ".$datostab->total; ?></a>
		    </li>
		<?php  } ?>
	  	</ul>	
		<div id="panel-body">
					...
					if ($datos) {
						foreach ($datos as $row) { 
							...
							<tr>
										...
										<td><a href="<?php echo base_url('index.php/gestiontransaccion/vertransaccion/'.$row->tg_numero_ref_n_pk); ?>" title='ver'><span class='glyphicon glyphicon-eye-open'></span></a><!--href='gestiontransaccion/gestiontransaccion/<?php echo $row->tg_numero_ref_n_pk; ?>'-->
						...
						<tr>
							<td colspan="6" align="center">No hay Transacciones en estatus <?php echo $estatus ?></td>
							</tr>
						<?php } ?>
					<tr>
