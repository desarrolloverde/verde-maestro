...
class Transacciongc_model extends CI_Model {
                ...
                if ($id === FALSE) {                                        
                    ...
                    $query = $this->db->query("SELECT tg_numero_ref_n_pk, tg_porcentaje_fee_n, tg_numero_tarjeta_a, tg_monto_fee_us_n, 
                       ...
                       gt_ruta_recibo_transf AS fpath
                        ...
                        WHERE as_estatus_b = TRUE ORDER BY gt_id_estatus_a DESC");
                    return $query->result();
                } else {
                    $query = $this->db->query("SELECT tg_numero_ref_n_pk, tg_porcentaje_fee_n, tg_numero_tarjeta_a, tg_monto_fee_us_n, 
                       tg_monto_disp_us_n, tg_promo_real_us_n, tg_monto_total_bs_n, 
                       ...
                       gt_ruta_recibo_transf AS fpath
                        ...
                        INNER JOIN vc_m_us_administrativos H ON G.as_id_usuario_a_pk=H.us_id_usuario_a_pk
                        INNER JOIN vc_m_gestion_transferencia I ON G.as_id_asignacion=I.gt_id_asignacion
                        WHERE as_estatus_b = TRUE and tg_numero_ref_n_pk='".$id."' ORDER BY gt_id_estatus_a DESC");
                    return $query->result();
                }               
                //result_array(); 
        }
        ...
        *vargason
        */
        public function getcountTransaccionesxEstatuscliente($cliente) {
            ...
            $query = $this->db->query("SELECT     (CASE
                                        ...
                                        as status, 
                                    count(tg_codigo_estatus_i) as total
                                    FROM vc_m_transaccion_giftcard INNER JOIN  vc_sesiones_vrcd ON tg_id_sesion_a = ss_id_sesion_a_pk       
                                    RIGHT JOIN vc_m_estatus_transaccion ON et_codigo_estatus_i = tg_codigo_estatus_i
                                    WHERE ss_us_verumcard_a_pk = '".$cliente."'
                                    GROUP BY id, status
                                    ORDER BY id ASC");
        ...
        public function getTransacciongcxStatusxpagina($idestatus,$limit, $start) 
            ...
            public function getTransacciongcxStatusxpaginaxusuario($idestatus,$limit, $start,$idusuario) 
            {
                $asignado=($idestatus==1) ? "" : " AND as_estatus_b=TRUE";
                $query = $this->db->query("SELECT           tg_numero_ref_n_pk, tg_porcentaje_fee_n, tg_numero_tarjeta_a, tg_monto_fee_us_n, 
                       tg_monto_disp_us_n, tg_promo_real_us_n, tg_monto_total_bs_n, 
                       ...
                       H.us_nombre_a ||' '||H.us_apellido_a as Asignado, H.us_id_usuario_a_pk as idusad     
                        ...
                        INNER JOIN vc_m_reg_cuentas_envio E on A.tg_cuenta_envio_a=E.rc_id_cuenta_a_pk
                        INNER JOIN vc_m_bancos F on E.rc_id_banco_a=F.bc_id_banco_a_pk
                        INNER JOIN vc_sesiones_vrcd I on A.tg_id_sesion_a = I.ss_id_sesion_a_pk
                        LEFT JOIN vc_m_asigna_transferencia G on A.tg_numero_ref_n_pk=G.as_numero_ref_n_pk
                        LEFT JOIN vc_m_us_administrativos H ON G.as_id_usuario_a_pk=H.us_id_usuario_a_pk
                        WHERE  tg_codigo_estatus_i in (".$idestatus.")".$asignado." AND I.ss_us_verumcard_a_pk='".$idusuario."' order by tg_fe_registro_t "." LIMIT ".$limit." OFFSET ".$start);
                    //return $query->result();
                //$this->db->limit($limit, $start);
                //$query = $this->db->get("users");
